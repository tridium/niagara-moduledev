  ...
  function modulePathToModuleDev(modInfo) {
    ...
    function doResolve(modulePath) {
      return (function fromProfile(i) {
        ...
        const buildDirPath = path.join(dir, moduleName + profiles[i], 'build', srcFolder, modulePath);
        return verifyFileIsReadable(buildDirPath)
          .catch(() => {
            const srcDirPath = path.join(dir, moduleName + profiles[i], srcFolder, modulePath);
            return verifyFileIsReadable(srcDirPath);
          })
